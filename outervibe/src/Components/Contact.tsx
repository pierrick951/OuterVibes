...
function Contact() {
  return (
    <div id="contact" className="px-5 lg:px-36 xl:px-40 w-full flex py-20 text-start flex-col justify-center items-center">
      <div className="bg-white rounded-xl shadow-lg p-6 mt-4 flex flex-col h-auto w-full max-w-6xl">
        ...
        <div className="flex flex-col items-center justify-center w-full md:w-3/4 lg:w-1/2 ">
            <form action={formAction} className="flex flex-col gap-4 w-full lg:max-w-sm">
              {contentForm.map((item) => (
                <div key={item.id} className="flex flex-col w-full">
                  <label htmlFor={item.label} className="text-gray-700 font-bold py-2">
                    {item.label}
                  </label>
                  <input
                    required
              ...
              <div className="text-center py-4">
                <button className="bg-red-500 py-2 px-5 hover:bg-red-300 rounded-xl text-white font-bold ">
                  {buttonSubmit}
                </button>
              </div>
            </form>
